...
while True:
  ...
  for t in range(20):
    ...
    # add the readings to the lists
    airpressureReadings.append(bmp180_airpressure)
    temperatureBmp180Readings.append(bmp180_temperature)
    ...
    # add the readings to the lists
    pm10Readings.append(pm10)
    pm25Readings.append(pm25)
  ...
  payload = '{{"sensor":"{:s}","timestamp":"{:s}","temperature":{:f},"humidity":{:f},"pm25":{:f},"pm10":{:f},"bmp180_temperature":{:f},"bmp180_airpressure":{:f}}}'.format(sensor_id, get_local_timestamp(),meanTemperature, meanHumidity,meanPM25,meanPM10,meanBmp180Ttemperature,meanAirpressure)
  try:
